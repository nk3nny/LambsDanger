...
if (_assignedVehicles isEqualTo []) then {
    ...
    _vehicles = _vehicles select {_x call EFUNC(main,isAlive) && canMove _x && simulationEnabled _x && {(crew _x) isEqualTo []} && { !isObjectHidden _x } && { locked _x != 2 } && {(getNumber (configOf _x >> "side")) isEqualTo _side}};
