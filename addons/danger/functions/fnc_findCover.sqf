...
if (GVAR(debug_functions) && {!(_ret isEqualTo [])}) then {
    ...
    {
        createVehicleLocal ["Sign_Arrow_Large_F", (_enemy call CBA_fnc_getPos) vectorAdd [0, 0, 1.8], [], 0, "CAN_COLLIDE"];
        ...
        createVehicleLocal ["Sign_Arrow_Large_Blue_F", (_x select 0) vectorAdd [0, 0, _add], [], 0, "CAN_COLLIDE"];
    } forEach _ret;
};
