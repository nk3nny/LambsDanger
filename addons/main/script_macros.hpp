...
// #define DISABLE_COMPILE_CACHE
    ...
    #undef PREP
    #define PREP(fncName) DFUNC(fncName) = compile preprocessFileLineNumbers QPATHTOF(functions\DOUBLES(fnc,fncName).sqf);uiNamespace setVariable [QUOTE(DFUNC(fncName)), DFUNC(fncName)]
#else
    #undef PREP
...
#ifdef DISABLE_COMPILE_CACHE
    #undef SUBPREP
    #define SUBPREP(sub,fncName) DFUNC(fncName) = compile preprocessFileLineNumbers QPATHTOF(functions\sub\DOUBLES(fnc,fncName).sqf);uiNamespace setVariable [QUOTE(DFUNC(fncName)), DFUNC(fncName)]
#else
    #undef SUBPREP
